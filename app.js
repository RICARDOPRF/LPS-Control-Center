...
const state = { authUser:null, person:null, projects:[], visibleProjects:[], people:[] };
...
async function currentPerson(user){
  ...
  const snap = await getDoc(doc(db,'people',email));
  return snap.exists() ? {id:snap.id,...snap.data()} : null;
}

async function audit(action, details='', projectId=null){
  try{
    if(!state.authUser || !state.person?.active) return;
...
function renderUser(){
  ...
  $('roleBadge').textContent = role; $('statRole').textContent = role;
}
...
async function loadProjects(){
  ...
  if(isSuper()){
    ...
    const out=[];
    for(const id of ids){ const s=await getDoc(doc(db,'projects',id)); if(s.exists()) out.push({id:s.id,...s.data()}); }
    state.projects=out; state.visibleProjects=out.filter(p=>p.active!==false && can(p.id,'view'));
  }
  renderProjects();
}
...
async function loadAudit(){
  if(!isSuper()) return;
  try{
    const snap=await getDocs(query(collection(db,'audit'),orderBy('createdAt','desc'),limit(100)));
...
async function saveProject(event){
  ...
  const repo=$('projectRepo').value.trim();
  const existing=state.projects.find(x=>x.id===original);
  ...
  await setDoc(doc(db,'projects',id),payload,{merge:true}); await audit('project_saved',payload.name||id,id); $('projectDialog').close(); await loadProjects();
}
...
async function savePerson(event){
  ...
  if(!original) payload.createdAt=serverTimestamp();
  await setDoc(doc(db,'people',email),payload,{merge:true}); if(original&&original!==email) await deleteDoc(doc(db,'people',original));
...
async function importSeed(){
  ...
  for(const item of seed.projects||[]){ const ref=doc(db,'projects',item.id); const existing=await getDoc(ref); if(existing.exists()) continue; await setDoc(ref,{...item,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),updatedBy:normalizeEmail(state.authUser.email)}); count++; }
...
async function boot(user){
  ...
  try{
    ...
    if(!person||person.active!==true){ $('blockedText').textContent=`A conta ${user.email||''} ainda não foi autorizada no LPS Control Center.`; showOnly('blocked'); return; }
    adminUI(); renderUser(); await loadProjects(); showOnly('app'); await audit('login','Acesso autenticado ao Control Center');
  }catch(err){ console.error(err); $('blockedText').textContent='Não foi possível validar suas permissões. Verifique o Firestore e as regras de acesso.'; showOnly('blocked'); }
}

$('googleLogin').addEventListener('click',async()=>{ try{ setStatus('Validando...'); await signInWithPopup(auth,googleProvider); }catch(err){ setStatus(err?.message||'Falha no login.'); } });
$('emailForm').addEventListener('submit',async e=>{ e.preventDefault(); try{ setStatus('Validando...'); await signInWithEmailAndPassword(auth,normalizeEmail($('email').value),$('password').value); }catch(err){ setStatus(err?.message||'Falha no login.'); } });
$('resetPassword').addEventListener('click',async()=>{ const email=normalizeEmail($('email').value); if(!email){setStatus('Informe o e-mail primeiro.');return;} try{await sendPasswordResetEmail(auth,email);setStatus('E-mail de redefinição enviado.');}catch(err){setStatus(err?.message||'Falha ao enviar.');} });
$('logout').addEventListener('click',()=>signOut(auth)); $('blockedLogout').addEventListener('click',()=>signOut(auth));
